...
The bounded architecture/qualification surface now covers W0 through W10. Coverage means future agents
have package-scoped contracts and stop conditions; it is not evidence that any runtime, provider,
performance or product gate has passed.
...
| W8 / P14 | Generic edge/CLI/leaf adapter packet prepared; optional leaf profiles disabled. |
| W9 / P15 | Product Pulse contract prepared; corpus/baselines/Windows/policy unselected, 60 probes unavailable. |
| W10 / P16–P18 | Model/document/scale candidate packets prepared; all profiles disabled/unselected, G6 not accepted. |

## Mechanical acceptance required before W10 packet merge
...
- W10 owner and swarm packet sets match exactly: model provider, two workers, daemon, bridge,
  publication, pins and reclaimer;
...
- network, auto-download/update, training/learning, persistent input cache, generative authority,
  document execution/remote resources and active in-place schema changes remain structurally forbidden;
...
- generic client pairing/binding/protocol runtime evidence;
...
`search-ports` separately. Do not start a W1+ or optional package because its packet exists.
