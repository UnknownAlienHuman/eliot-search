...
The canonical marker codec, filename, temporary filename, maximum size,
operator receipts, cutover replay semantics and staged database schema are
unchanged. No dependency, lockfile or persisted format changed.
...
Read-only resolution now also validates that the `control/` directory itself is
a real admitted directory. A redirected or unstable parent fails closed as
...
`write_all`, `rename` and bounded `read_to_end` to remain in
`cutover_artifact.rs`. It rejects restoration of those operations in the daemon
marker adapter. Quarantine references are forbidden in the package owner and
required in the daemon adapter.
