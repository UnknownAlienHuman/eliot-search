...
#[cfg(test)]
impl TestWork {
    // `const` construction for the shared `const` journal constructor.
    const fn new() -> Self {
        Self {
            point_reads: std::sync::atomic::AtomicU64::new(0),
...
impl Boundary {
    // Production builds carry no fault-injection arms, so the fence is an
    // infallible `const` step. Test builds use the fallible helper below to
    // exercise every `Err`/exit arm through `OUTCOME_UNKNOWN` recovery.
    #[cfg(not(test))]
    const fn before_commit() {}

    #[cfg(not(test))]
    const fn after_commit() {}

    #[cfg(test)]
    fn before_commit(self) -> Result<(), ControlError> {
        match self {
            Self::Normal | Self::LostAcknowledgement | Self::ExitAfterCommit => Ok(()),
            Self::BeforeCommit => Err(ControlError::StoreUnavailable),
            Self::ExitBeforeCommit => std::process::exit(73),
        }
    }
    ...
    fn after_commit(self) -> Result<(), ControlError> {
        match self {
            Self::Normal | Self::BeforeCommit | Self::ExitBeforeCommit => Ok(()),
            Self::LostAcknowledgement => Err(ControlError::CommitOutcomeUnknown),
            Self::ExitAfterCommit => std::process::exit(74),
        }
    }
}
