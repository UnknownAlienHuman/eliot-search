...
The package also owns the frozen pure byte-layout and binding-validation contract for the legacy
DIRECT protected-revision envelope. That compatibility contract performs no DPAPI, credential,
filesystem, source-catalog, or plaintext-admission operation; the daemon injects the concrete content
digest and platform effect.
...
- Credential Manager, DPAPI, RNG, filesystem or process effects
