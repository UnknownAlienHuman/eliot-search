...
//!
//! The package performs no filesystem or database I/O. It consumes exact
//! retained bytes, preserves them without newline or Unicode normalization,
//! records source-byte line spans and line-ending evidence, and rejects binary,
//! malformed, empty, or over-limit input before downstream unitization.
...
impl RetainedRevision {
    /// Creates an exact retained revision value.
    #[must_use]
    pub const fn new(
        source_id: OpaqueId,
...
pub enum LineEnding {
    ...
    CrLf,
    /// Standalone carriage-return byte (`\r`).
    Cr,
...
pub struct MaterializationReceipt {
    ...
    /// Durable retained-revision receipt.
...
pub struct MaterializedRevision {
    ...
    /// Exact source content digest.
    pub content_digest: Blake3Digest32,
    /// Exact UTF-8 text without normalization.
    text: String,
...
/// Materializes exact retained bytes as strict UTF-8 without normalization.
pub fn materialize(
    ...
    let text = String::from_utf8(input.bytes)
        .map_err(|_| MaterializationError::InvalidUtf8)?;
...
fn scan_lines(
    ...
    while index < bytes.len() {
        ...
        lines.push(LineSpan {
            ...
            source_start: u64::try_from(start)
                .map_err(|_| MaterializationError::OffsetOverflow)?,
        ...
        match ending {
            LineEnding::Lf => evidence.lf = evidence.lf.saturating_add(1),
            LineEnding::CrLf => evidence.crlf = evidence.crlf.saturating_add(1),
            LineEnding::Cr => evidence.cr = evidence.cr.saturating_add(1),
            LineEnding::None => {}
        }
        start = source_end;
        index = source_end;
    ...
    if start < bytes.len() {
        ...
        evidence.unterminated = evidence.unterminated.saturating_add(1);
    }
    Ok((lines, evidence))
}
...
mod tests {
    ...
    fn line_endings_and_offsets_are_exact() {
        ...
        assert_eq!(result.lines[0].source_start, 0);
        assert_eq!(result.lines[0].content_end, 1);
        assert_eq!(result.lines[0].source_end, 3);
    ...
    fn debug_output_does_not_dump_source_text() {
        ...
        let result = materialize(input, DEFAULT_MATERIALIZATION_LIMITS)
            .expect("materialize");
        let debug = format!("{result:?}");
        assert!(!debug.contains("sensitive source text"));
