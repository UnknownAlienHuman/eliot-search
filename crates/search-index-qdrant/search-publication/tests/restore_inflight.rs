...
fn reference(value: &str) -> ReceiptRef { ReceiptRef::new(format!("fixture:{value}")).unwrap() }
fn point(value: u8) -> PointId128 { PointId128::from_bytes([value; 16]) }
fn guards() -> PublicationGuards {
    PublicationGuards { owner_epoch: OwnerEpoch::new(7).unwrap(), source_catalog_generation: 11,
        membership_generation: 13, access_generation: 17, shadow_generation: 19,
        purge_generation: 23, profile_digest: digest(29) }
}
fn manifest(points: &[u8]) -> ProjectionManifest {
    ProjectionManifest {
        entries: points.iter().copied().map(|n| ProjectionManifestEntry {
            point_id: point(n),
            ...
            source_membership_id: id("source-member"), projection_membership_id: id("projection-member"),
...
fn prepared(old: Option<ProjectionManifest>, name: &str) -> PreparedPublication {
    PreparedPublication {
        transaction_id: id(name), collection_generation_id: CollectionGenerationId::from_bytes([1; 16]),
        old_manifest_digest: old.as_ref().map(|_| digest(1)), old_manifest: old,
        new_manifest: manifest(&[2]), new_manifest_digest: digest(2), guards: guards(),
...
fn snapshot(commit: &VisibleCommitReceipt) -> SnapshotPublishReceipt {
    SnapshotPublishReceipt { transaction_id: commit.transaction_id.clone(), visible_epoch: commit.visible_epoch,
        control_generation: commit.control_generation, snapshot_digest: commit.control_state_digest }
}

/// Produce an accepted receipt prefix through the original public state machine.
fn history(level: u8, replacement: bool) -> (PublicationCoordinator, Option<ControlCommitObservation>) {
    ...
    let mut machine = PublicationCoordinator::new(previous, floor, old.clone(),
        old.as_ref().map(|_| digest(1)), 32).unwrap();
    machine.submit(prepared(old, "original-transaction")).unwrap();
    machine.persist_intent(id("original-intent-operation"), reference("original-intent")).unwrap();
    let transaction = machine.active().unwrap().clone();
    let empty = manifest(&[]);
    let difference = diff_manifests(transaction.prepared.old_manifest.as_ref().unwrap_or(&empty),
        &transaction.prepared.new_manifest).unwrap();
    let mut staged = difference.create.iter().map(|entry| entry.point_id).collect::<Vec<_>>();
    let mut closed = difference.retire.iter().map(|entry| entry.point_id).collect::<Vec<_>>();
    staged.sort(); closed.sort();
    if level >= 1 {
        machine.stage_new_points(StageReceipt { transaction_id: transaction.prepared.transaction_id.clone(),
            target_epoch: transaction.target_epoch, staged_ids: staged, missing_ids: vec![], unexpected_ids: vec![],
            readback_digest: digest(3), mutation_receipt: reference("original-stage") }).unwrap();
    }
    if level >= 2 {
        machine.close_old_points(ClosureReceipt { transaction_id: transaction.prepared.transaction_id.clone(),
            target_epoch: transaction.target_epoch, closed_ids: closed.clone(), missing_ids: vec![], unexpected_ids: vec![],
            readback_digest: digest(4), mutation_receipt: reference("original-closure") }).unwrap();
    }
    if level >= 3 {
        machine.verify_readback(digest(3), digest(4), (!closed.is_empty()).then(|| digest(5))).unwrap();
    }
    let mut observation = None;
    if level >= 4 {
        let commit = ControlCommitObservation { before_visible_epoch: previous, after_visible_epoch: transaction.target_epoch,
            observed_guards: guards(), control_generation: 31, control_state_digest: digest(6) };
        let receipt = machine.commit_visible_epoch(commit).unwrap();
        observation = Some(commit);
        if level >= 5 { machine.publish_control_snapshot(snapshot(&receipt)).unwrap(); }
    }
    (machine, observation)
}
...
fn input(level: u8, replacement: bool) -> PublicationRestoreInput {
    ...
    PublicationRestoreInput {
        prepared: transaction.prepared.clone(), previous_visible_epoch: transaction.previous_visible_epoch,
        last_reserved_epoch: machine.last_reserved_epoch(),
        collection_generation_id: transaction.prepared.collection_generation_id,
        current_guards: guards(), phase: transaction.phase,
        intent: transaction.durable_intent.clone().unwrap(),
        stage_receipt: transaction.stage_receipt.clone(), closure_receipt: transaction.closure_receipt.clone(),
        verified: transaction.verified.clone(), control_commit: commit,
        observation: PublicationRecoveryObservation {
            intent_durable: true,
            staged_ids: transaction.stage_receipt.as_ref().map_or_else(Vec::new, |r| r.staged_ids.clone()),
            closed_ids: transaction.closure_receipt.as_ref().map_or_else(Vec::new, |r| r.closed_ids.clone()),
            control_visible_epoch: machine.visible_epoch(),
            snapshot_published: false, abandon_fence_durable: false,
        },
    }
}
fn restore(input: PublicationRestoreInput) -> (PublicationCoordinator, PublicationRecoveryDecision) {
...
fn all_precommit_prefixes_restore_the_original_slot_and_original_operation_ids() {
    for level in 0..=3 {
        ...
        assert_eq!(restored.submit(prepared(None, "competing")), Err(PublicationError::PublicationBusy));
        assert_eq!(restored.last_reserved_epoch(), epoch(1));
    }
}
...
fn restored_verified_transaction_can_finish_through_the_existing_commit_and_snapshot_path() {
    ...
    let commit = restored.commit_visible_epoch(ControlCommitObservation {
        before_visible_epoch: epoch(0), after_visible_epoch: epoch(1), observed_guards: guards(),
        control_generation: 31, control_state_digest: digest(6),
    }).unwrap();
    assert_eq!(restored.complete(), Err(PublicationError::InvalidTransition));
    restored.publish_control_snapshot(snapshot(&commit)).unwrap();
...
fn both_committed_phases_need_a_new_current_process_snapshot_acknowledgement() {
    for level in [4, 5] {
        ...
        assert_eq!(restored.active().unwrap().phase, PublicationPhase::ControlCommitted);
...
fn a_previous_process_snapshot_flag_is_not_accepted_as_fresh_evidence() {
    ...
    assert_eq!(PublicationCoordinator::restore_inflight(request, 32).unwrap_err(), PublicationError::RecoveryBlocked);
}
...
fn partial_effects_restore_two_sided_compensation_and_preserve_consumed_gaps() {
    ...
    assert_eq!(restored.active().unwrap().phase, PublicationPhase::Compensating);
    ...
    let removed = CompensationReceipt { transaction_id: plan.transaction_id.clone(), target_epoch: plan.target_epoch,
        compensated_ids: plan.staged_ids.clone(), remaining_ids: vec![], readback_receipt: reference("removed") };
    assert_eq!(restored.compensate_exact(removed.clone()), Err(PublicationError::CompensationIncomplete));
    restored.compensate_and_restore(removed, RestorationReceipt {
...
fn every_changed_live_guard_prevents_restored_forward_commit() {
    for axis in 0..7 {
        ...
        assert_eq!(restored.commit_visible_epoch(ControlCommitObservation {
            before_visible_epoch: epoch(0), after_visible_epoch: epoch(1), observed_guards: guards(),
            control_generation: 31, control_state_digest: digest(6),
...
fn an_older_owner_or_foreign_collection_cannot_hydrate_the_transaction() {
    ...
    assert_eq!(PublicationCoordinator::restore_inflight(old_owner, 32).unwrap_err(), PublicationError::GuardMismatch);
    let mut route = input(0, false);
    route.collection_generation_id = CollectionGenerationId::from_bytes([2; 16]);
    assert_eq!(PublicationCoordinator::restore_inflight(route, 32).unwrap_err(), PublicationError::OperationMismatch);
}
...
fn unresolved_target_must_equal_the_consumed_floor_not_visible_plus_one() {
    for floor in [0, 8, 10] {
        ...
        assert_eq!(PublicationCoordinator::restore_inflight(request, 32).unwrap_err(), PublicationError::EpochMismatch);
    }
...
fn only_the_recorded_phases_complete_receipt_prefix_is_accepted() {
    let phases = [PublicationPhase::IntentDurable, PublicationPhase::NewPointsAcknowledged,
    ...
    for (phase, valid_mask) in phases.into_iter().zip([0, 1, 3, 7, 15, 15]) {
        for mask in 0..16 {
            if mask == valid_mask { continue; }
            let mut request = input(4, false);
            request.phase = phase;
            if mask & 1 == 0 { request.stage_receipt = None; }
            if mask & 2 == 0 { request.closure_receipt = None; }
            if mask & 4 == 0 { request.verified = None; }
            if mask & 8 == 0 { request.control_commit = None; }
            assert_eq!(PublicationCoordinator::restore_inflight(request, 32).unwrap_err(), PublicationError::RecoveryBlocked);
        }
    }
}
...
fn receipt_ids_epochs_and_combined_digests_are_revalidated_by_the_existing_transitions() {
    for case in 0..12 {
        ...
        match case {
            ...
            2 => request.stage_receipt.as_mut().unwrap().staged_ids.push(point(2)),
            3 => request.closure_receipt.as_mut().unwrap().closed_ids.clear(),
            4 => request.closure_receipt.as_mut().unwrap().unexpected_ids.push(point(9)),
            ...
            10 => request.control_commit.as_mut().unwrap().observed_guards.purge_generation += 1,
...
fn fresh_control_visibility_must_match_the_validated_committed_or_precommit_state() {
    for (level, wrong_epoch) in [(0, 1), (3, 1), (4, 0), (4, 2)] {
        ...
        assert_eq!(PublicationCoordinator::restore_inflight(request, 32).unwrap_err(), PublicationError::ControlConflict);
    }
}

#[test]
fn restored_aborted_and_blocked_records_keep_the_active_slot_unavailable() {
    for phase in [PublicationPhase::Aborted, PublicationPhase::PublicationBlocked] {
        let mut request = input(3, true);
        request.phase = phase;
        let (mut restored, decision) = restore(request);
        assert_eq!(decision, PublicationRecoveryDecision::PublicationBlocked);
        assert_eq!(restored.active().unwrap().phase, PublicationPhase::PublicationBlocked);
        assert!(restored.finalize_aborted().is_err());
        assert!(restored.complete().is_err());
        assert_eq!(restored.submit(prepared(Some(manifest(&[1])), "competing")), Err(PublicationError::PublicationBusy));
        assert_eq!(restored.last_reserved_epoch(), epoch(9));
    }
}
...
fn recorded_compensating_never_returns_to_forward_progress_even_when_all_ids_are_present() {
    ...
    assert_eq!(decision, PublicationRecoveryDecision::CompensateExact);
    assert_eq!(restored.active().unwrap().phase, PublicationPhase::Compensating);
    assert_eq!(restored.begin_compensation_plan().unwrap().closed_ids, vec![point(1)]);
}
...
fn conflicting_physical_ids_and_unpaired_manifests_cannot_enter_rehydration() {
    ...
    assert_eq!(PublicationCoordinator::restore_inflight(conflict, 32).unwrap_err(), PublicationError::InvalidPreparedPublication);
    let mut unpaired = input(0, true);
    unpaired.prepared.old_manifest_digest = None;
    assert_eq!(PublicationCoordinator::restore_inflight(unpaired, 32).unwrap_err(), PublicationError::InvalidPreparedPublication);
}
...
fn finite_point_limits_cover_receipts_and_observations_before_reconstruction() {
    ...
    for case in 0..8 {
        ...
        assert_eq!(PublicationCoordinator::restore_inflight(request, 32).unwrap_err(), PublicationError::BudgetExceeded);
    }
}
...
fn unverifiable_fences_and_incomplete_committed_readback_return_a_blocked_not_ready_slot() {
    for case in 0..2 {
        let mut request = input(4, true);
        if case == 0 { request.observation.abandon_fence_durable = true; }
        else { request.observation.staged_ids.clear(); }
        let (mut restored, decision) = restore(request);
        assert_eq!(decision, PublicationRecoveryDecision::PublicationBlocked);
...
fn restore_input_debug_does_not_print_manifests_or_receipt_identifiers() {
    ...
    for sentinel in ["original-transaction", "original-preparation", "original-intent-operation",
        "original-stage", "original-closure", "fixture:manifest-not-CAS-evidence", "source-member"] {
