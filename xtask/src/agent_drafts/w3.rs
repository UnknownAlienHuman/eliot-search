...
#[must_use]
pub fn validate_w3_agent_drafts(root: &Path) -> W3AgentDraftReport {
    match validate(root) {
        Ok(report) => report,
        Err(error) => W3AgentDraftReport::early(error),
    }
}

#[must_use]
pub const fn exit_code(report: &W3AgentDraftReport) -> i32 {
...
fn validate(root: &Path) -> Result<W3AgentDraftReport, String> {
    ...
    let context_manifest = load_doc(root, "swarm/context-drafts/w3/manifest.toml")?;
    ...
    let cases = load_doc(root, "qualification/w3-agent-drafts/cases-v1.toml")?;
