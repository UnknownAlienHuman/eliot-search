...
use super::super::{
    boolean, child_bool, child_string, child_string_list, expected_strings,
    integer, require_regular_file, string, string_list,
};
...
fn validate_registries(
    ...
    let expected_wave = if spec.name == "eliot-searchd" { 1 } else { 4 };
...
fn validate_ticket(
    ...
    if string(ticket, "stage") != Some("W4")
        || integer(ticket, "wave") != Some(4)
    ...
    if child_string(ticket, "qualification", "status") != Some("DESIGNED_NOT_EXECUTED")
        || child_string(ticket, "qualification", "mandatory_probe_evidence")
            != Some("UNAVAILABLE")
        || child_string(ticket, "qualification", "independent_reviewer_receipt")
            != Some("ABSENT")
        || child_bool(ticket, "qualification", "query_product_enabled") != Some(false)
    {
        errors.push(format!(
            "{}: qualification state is successful or incomplete",
            spec.name
        ));
    }
}
...
fn validate_context(
    ...
    if string(context, "stage") != Some("W4")
        || integer(context, "wave") != Some(4)
    ...
    let selectors = child_string_list(context, "content", "registry_fragments")
        .unwrap_or_default();
    ...
    let expected_selectors = vec![
        ...
        "swarm/stages.toml::stage[id=W4]".to_owned(),
    ];
    ...
    for source in &sources {
        ...
        if matches!(
            ...
            "docs/handoff/W1_IMPLEMENTATION_PACKET.md"
                ...
                | "docs/handoff/W3_IMPLEMENTATION_PACKET.md"
        ) {
    ...
    if !sources
        .iter()
        .any(|source| source == "qualification/query/W4_QUALIFICATION.md")
    {
        errors.push(format!(
            "{}: W4 qualification contract absent from context",
            spec.name
        ));
    }
    ...
    if spec.name == "eliot-searchd"
        && !sources
            .iter()
            .any(|source| source == "docs/handoff/W4_DAEMON_REENTRY.md")
    {
