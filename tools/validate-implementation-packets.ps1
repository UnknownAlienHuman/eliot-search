...
if (-not $packages.Contains("search-config")) { Add-Error "search-config is absent from package registry." }
...
for ($i = 1; $i -lt $sectionBlocks.Count; $i++) {
    ...
    if ($contractText -and $contractText -notmatch ('(?m)^\- \*\*Owner:\*\* `' + [regex]::Escape($owner) + '`$')) {
        Add-Error "Configuration packet '$contract' does not declare owner '$owner'."
    }
    foreach ($term in @("## Fields", "## Required section API", "## Invariants", "## Required tests")) {
        if ($contractText -and -not $contractText.Contains($term)) {
            Add-Error "Configuration packet '$contract' lacks '$term'."
        }
    }
}
...
foreach ($package in $packages.Values) {
    ...
    if ($package.Qualification) {
        [void](Read-Required $package.Qualification)
    }
    ...
    foreach ($sectionName in $package.ConfigSections) {
        ...
        if ($section.Owner -cne $package.Name) {
            Add-Error "Configuration owner mismatch for '$sectionName': registry package=$($package.Name), section owner=$($section.Owner)."
        }
        if ($package.Wave -gt $section.FirstWave) {
            Add-Error "Configuration section '$sectionName' begins at W$($section.FirstWave) before owner $($package.Name) W$($package.Wave)."
        }
        if ($package.Deps -cnotcontains "search-config" -and $package.Name -cne "search-config") {
...
if (-not (Same-Set $exampleSections @($sections.Keys))) {
    Add-Error "Example config sections differ from the configuration registry."
}
$activeExampleLines = @(
    $exampleText -split '\r?\n' |
        Where-Object { $_ -notmatch '^\s*#' -and $_ -notmatch '^\s*$' }
)
foreach ($line in $activeExampleLines) {
...
if ($exampleText -match '(?m)^\s*auto_(download|upgrade)\s*=\s*true\s*$') {
    Add-Error "Example config enables automatic Qdrant download/upgrade."
}
...
if ($exampleText -notmatch '(?ms)^\[qdrant_process\].*?^enabled\s*=\s*false\s*$') {
    Add-Error "Example config must keep qdrant_process disabled."
}
...
foreach ($file in $qualificationRequired) { [void](Read-Required $file) }

$artifact = Read-Required "qualification/qdrant/artifact.toml"
...
if ($artifact -and (Get-TomlBool $artifact "automatic_download") -ne $false) {
    Add-Error "Qdrant artifact contract permits automatic download."
}
if ($artifact -and (Get-TomlBool $artifact "automatic_upgrade") -ne $false) {
    Add-Error "Qdrant artifact contract permits automatic upgrade."
}
$status = if ($artifact) { Get-TomlString $artifact "status" } else { "" }
...
if ($status -eq "QUALIFIED") {
    foreach ($key in @("qualified_at", "qualification_receipt_ref", "reviewer_receipt_ref")) {
        if ([string]::IsNullOrWhiteSpace((Get-TomlString $artifact $key))) {
            Add-Error "QUALIFIED artifact lacks '$key'."
        }
    }
}
...
foreach ($term in @(
    ...
    if ($collectionSchema -and -not $collectionSchema.Contains($term)) {
        Add-Error "Collection schema lacks required term '$term'."
    }
}

$launchCount = [int](Get-TomlInt $launchText "scaffold_package_count")
$launchLibs = [int](Get-TomlInt $launchText "library_package_count")
$launchBins = [int](Get-TomlInt $launchText "binary_package_count")
if ($launchCount -ne 45 -or $launchLibs -ne 41 -or $launchBins -ne 4) {
    Add-Error "Launch state is not synchronized to 45/41/4."
}
...
$result = [ordered]@{
    ...
    function_packets = @($packages.Values | Where-Object { $_.Functions }).Count
