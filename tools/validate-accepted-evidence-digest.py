...
def main() -> int:
    ...
    try:
        ...
        cases = tomllib.loads((ROOT / REQUIRED[5]).read_text(encoding="utf-8"))
    ...
    if cases.get("case_count") != 10 or len(case_rows) != 10:
        errors.append("accepted evidence case inventory must contain exactly ten cases")
    case_ids = [row.get("id") for row in case_rows if isinstance(row, dict)]
    if len(set(case_ids)) != len(case_ids):
        errors.append("accepted evidence case IDs must be unique")
