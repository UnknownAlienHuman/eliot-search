...
if (-not (Same-Set $expectedPackages $owners.ToArray())) { Add-Error 'W8 manifest owner set differs from expected packages.' }
...
if (-not (Same-Set $expectedPackages $packetPackages.ToArray())) { Add-Error 'W8 packet package set differs from manifest owners.' }
...
Require-Tokens 'crates/search-research-export-adapter/FUNCTIONS.md' (Read-Text 'crates/search-research-export-adapter/FUNCTIONS.md') @(
    ...
    '3a5f9fd2b254eebe574b2c4a28f9804df0da9df359e59ceee125fa7da90fef22', 'Required tests'
)
...
foreach ($key in @('eliot_adapter.enabled', 'research_export.enabled')) {
    if (-not $fields.ContainsKey($key) -or $fields[$key].DefaultRaw -cne 'false') { Add-Error "$key must default false." }
}
foreach ($key in @('eliot_adapter.compiled_feature_ref', 'eliot_adapter.mapping_profile_ref', 'research_export.compiled_feature_ref')) {
    if (-not $fields.ContainsKey($key) -or $fields[$key].Mode -cne 'QUALIFIED_REF' -or $fields[$key].DefaultRaw -cne '"UNSELECTED"') {
...
for ($i = 1; $i -lt $probeBlocks.Count; $i++) {
    ...
    if ($profile -ceq 'generic') {
        $genericCount++
        if (-not $mandatory -or $result -cne 'UNAVAILABLE') { Add-Error "Generic probe $id must be mandatory UNAVAILABLE." }
    } else {
        $optionalCount++
        if ($mandatory -or $result -cne 'DISABLED') { Add-Error "Optional probe $id must be non-mandatory DISABLED." }
    }
...
if (-not (Same-Set $expectedEvidence $evidenceIds.ToArray())) { Add-Error 'Gate-map evidence set differs from central G4 IDs.' }
