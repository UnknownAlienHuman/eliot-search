...
for ($i = 1; $i -lt $probeBlocks.Count; $i++) {
    ...
    $probeResult = Get-TomlString $block 'result'
    $probeCount++
    ...
    if ($probeResult -cne 'UNAVAILABLE') { Add-Error "W6 probe '$id' must remain UNAVAILABLE before execution." }
}
...
if (-not $g3BlockMatch.Success) { Add-Error 'G3 gate block is missing.' }
    ...
    foreach ($evidenceId in @(
        ...
        if ($g3.IndexOf(('"' + $evidenceId + '"'), [StringComparison]::Ordinal) -lt 0) {
