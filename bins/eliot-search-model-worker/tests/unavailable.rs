//! Boundary: the optional model worker is honestly unavailable by default.
//!
...
fn run(args: &[&str], extra_env: &[(&str, &str)]) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_eliot-search-model-worker"));
    command.args(args).stdin(Stdio::null());
    for (key, value) in extra_env {
        command.env(key, value);
    }
    command.output().expect("spawn model worker binary")
}

fn assert_refusal(output: &Output) {
    assert!(
        !output.status.success(),
        "model worker must never exit 0 while unqualified"
    );
    assert_eq!(
        output.status.code(),
        Some(EXPECTED_CODE),
        "model worker refusal must use the pinned nonzero exit"
    );
    assert!(
    ...
    assert!(
        stderr.contains("MODEL_WORKER_PROFILE_DISABLED"),
...
fn forced_env_refuses_with_same_announcement() {
    let forced = run(&[], &[("ELIOT_SEARCH_MODEL_WORKER_FORCE", "1")]);
    let baseline = run(&[], &[]);
    assert_refusal(&forced);
    assert_eq!(
        forced.stderr, baseline.stderr,
