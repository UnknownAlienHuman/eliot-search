//! Human-readable client for the owner-fenced DIRECT stdio service.
//!
//! The daemon protocol is tab/hex framed. This client accepts bounded UTF-8
//! command lines, encodes query and native path arguments, and streams
...
fn run_session(daemon: &Path, root: &Path) -> Result<(), String> {
    ...
    loop {
        ...
        match translate_command(&line) {
            Ok(ClientAction::Help) => print_session_help(),
...
fn drain_until_terminal(reader: &mut impl BufRead, terminal: Terminal) -> Result<(), String> {
    loop {
...
fn translate_command(line: &str) -> Result<ClientAction, String> {
    ...
    let translated = match verb {
        ...
        "list-sources" if remainder.is_empty() => TranslatedCommand {
            protocol: "list-sources".to_owned(),
            terminal: Terminal::SourceList,
        },
        "shutdown" if remainder.is_empty() => TranslatedCommand {
        ...
        "index-directory" => TranslatedCommand {
            protocol: format!(
                "index-directory\t{}",
                encode_path(Path::new(required_remainder(
                    remainder,
        ...
        "sync-directory" => TranslatedCommand {
            protocol: format!(
                "sync-directory\t{}",
                encode_path(Path::new(required_remainder(
                    remainder,
        ...
        "search" | "search-i" => {
            ...
            if query.len() > MAX_QUERY_BYTES {
                return Err("SEARCH_QUERY_TOO_LARGE".to_owned());
            }
