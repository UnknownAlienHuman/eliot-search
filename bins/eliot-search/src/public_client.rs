//! Human-readable client for paged DIRECT search with opaque source handles.
//!
//! The client tracks only the latest session continuation and latest opaque
//! source handle. It never receives internal source, revision, path, or content
//! identities from the default paged search path.
...
fn run_session(daemon: &Path, root: &Path) -> Result<(), String> {
    ...
    let mut latest_source_handle: Option<String> = None;
    loop {
        ...
        let action = match translate_command(
            &line,
            latest_continuation.as_deref(),
            latest_source_handle.as_deref(),
        ) {
            Ok(action) => action,
            Err(error) => {
                eprintln!(
        ...
        match action {
            ...
            ClientAction::Protocol(translated) => {
                ...
                if succeeded && translated.clears_search_state {
                    latest_continuation = None;
                    latest_source_handle = None;
                }
...
struct ResponseSummary {
    ...
    latest_source_handle: Option<String>,
}
...
fn drain_until_terminal(
    ...
    let mut latest_source_handle = None;
    loop {
        let line = read_response_line(reader)?
            .ok_or_else(|| "DAEMON_CLOSED_MID_RESPONSE".to_owned())?;
        println!("{line}");
        if let Some(handle) = extract_optional_token(&line, "source_handle")? {
            latest_source_handle = Some(handle);
        }
        if terminal.reached(&line) {
            return Ok(ResponseSummary {
                terminal_line: line,
                latest_source_handle,
            });
        }
    }
}
...
fn translate_command(
    ...
    let translated = match verb {
        ...
        "shutdown" if remainder.is_empty() => command(
            "shutdown".to_owned(),
            Terminal::Shutdown,
            true,
            false,
...
fn parse_range(value: &str) -> Result<(u64, u64), String> {
    ...
    if fields.len() != 2 {
        return Err("EXPAND_LAST_USAGE".to_owned());
    }
    Ok((
        parse_u64(fields[0], "HANDLE_START_INVALID")?,
        parse_u64(fields[1], "HANDLE_END_INVALID")?,
    ))
}
...
fn validate_token(value: &str) -> Result<(), String> {
    if value.len() == 64 && value.as_bytes().iter().all(u8::is_ascii_hexdigit) {
        ...
        Err("HANDLE_TOKEN_INVALID".to_owned())
    }
}
...
fn single(protocol: &str) -> TranslatedCommand {
    command(protocol.to_owned(), Terminal::Single, false, false)
}

fn extract_optional_token(line: &str, field: &str) -> Result<Option<String>, String> {
    ...
    let remainder = remainder
        .strip_prefix('"')
        .ok_or_else(|| "TOKEN_RESPONSE_INVALID".to_owned())?;
    let end = remainder
        .find('"')
        .ok_or_else(|| "TOKEN_RESPONSE_INVALID".to_owned())?;
...
fn print_session_help() {
    eprintln!(
        concat!(
            ...
            "next [SIZE], continue TOKEN [SIZE], expand-last START END, ",
            "expand-handle TOKEN START END, search-all QUERY, search-all-i QUERY, ",
            "retire SOURCE_ID, read-revision REVISION_ID START END, ",
            "gc-dry-run, gc-apply, shutdown"
        )
    );
}
