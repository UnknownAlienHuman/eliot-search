...
impl DirectStore {
    ...
    pub(crate) fn inspect_migration_preparation_files(
        ...
        let checkpoint = sha256::digest_parts(
            b"eliot-search/preparation-inventory-cursor/v1",
            &[
                &catalog,
                &profile,
        ...
        if first > inventory.files.len()
            || cursor
                .as_ref()
                .is_some_and(|value| value.checkpoint != checkpoint)
        {
        ...
        for entry in inventory.files.values().skip(first) {
            ...
            let digest = sha256::digest(&read_artifact(
                &self.root,
                entry,
                MAX_OBJECT_BYTES,
                deadline,
            ...
            rows.push(format!(
                ...
                json_string(&legacy_preparation_rooted_locator(&entry.relative)),
                json_string(entry.kind.tag()),
                entry.size,
                sha256::hex(&digest),
                entry.target.as_ref().map_or_else(
                    || "null".to_owned(),
                    |target| json_string(&legacy_preparation_rooted_locator(target)),
                ),
            ));
        ...
        let page_digest = sha256::digest_parts(
            b"eliot-search/preparation-inventory-page/v1",
            &[
                &checkpoint,
                &(first as u64).to_be_bytes(),
                &(next as u64).to_be_bytes(),
                body.as_bytes(),
            ],
        );
    ...
    fn preparation_inventory(&self, deadline: Instant) -> Result<Inventory, String> {
        ...
        for metadata in self.inner.retained_revisions() {
            ...
            if entry.kind != Kind::UnmappedReference {
                return Err(invalid());
            }
        ...
        for entry in inventory.files.values() {
            ...
            digest = sha256::digest_parts(
                b"eliot-search/preparation-inventory-file/v1",
                &[
                    &digest,
                    entry.relative.as_bytes(),
                    &entry.size.to_be_bytes(),
                    &modified.as_secs().to_be_bytes(),
                    &modified.subsec_nanos().to_be_bytes(),
...
fn stamp(metadata: &Metadata) -> Result<(u64, SystemTime), String> {
    ...
    if !metadata.is_file()
        ...
        || reparse
        || metadata.len() > MAX_OBJECT_BYTES as u64
    {
        return Err(invalid());
    }
    Ok((
...
fn read_artifact(
    ...
    if bytes.len() as u64 != entry.size
        ...
        || entry
            .reference
            .as_ref()
            .is_some_and(|saved| saved.as_slice() != bytes.as_slice())
    {
        return Err("DIRECT_MIGRATION_PREPARATION_OBJECT_CHANGED".to_owned());
    }
    check(deadline)?;
