...
impl SocketDeadline {
    ...
    pub(super) fn read_line(
        ...
        if maximum_bytes == 0 || maximum_bytes > MAX_COMMAND_LINE_BYTES {
        ...
        loop {
            ...
            if reader.buffer().is_empty() {
                reader
                    .get_ref()
                    .set_read_timeout(Some(self.remaining(READ_TIMEOUT).map_err(read_error)?))
                    .map_err(|error| {
                        redacted_io_error("ENDPOINT_TIMEOUT_CONFIGURATION_ERROR", &error)
                    })?;
            }
            let (consumed, ended) = {
            ...
            // A full frame without LF cannot become valid. Do not wait for an
            // overflow byte, drain the suffix or dispatch its apparent command.
